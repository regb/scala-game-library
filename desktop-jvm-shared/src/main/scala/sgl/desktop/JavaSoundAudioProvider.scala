...
/** AudioProvider implementation for the official Java Sound API.
  ...
  */
trait JavaSoundAudioProvider extends AudioProvider {
  this: DesktopSystemProvider with LoggingProvider =>

  import JavaSoundHelpers._
  
  private implicit val LogTag: JavaSoundAudioProvider.this.Logger.Tag = Logger.Tag("sgl-java-sound-audio")

  object JavaSoundAudio extends Audio {
    ...
    override def loadSound(asset: sgl.assets.AudioAsset, extras: sgl.assets.AudioAsset*): Loader[Sound] = FutureLoader {
      loadFirstSupportedSound(asset +: extras.toSeq)
    }

    private def loadFirstSupportedSound(assets: Seq[sgl.assets.AudioAsset]): Sound = {
      var lastFailure: Option[Exception] = None
      val remainingAssets = assets.iterator
      var loadedSound: Option[Sound] = None
      while(loadedSound.isEmpty && remainingAssets.hasNext) {
        val asset = remainingAssets.next()
        try {
          loadedSound = Some(loadSingleSound(asset))
        } catch {
          case e @ (_: ResourceNotFoundException | _: ResourceFormatUnsupportedException) =>
            lastFailure = Some(e)
            logger.warning("Unable to load sound resource %s: %s. Trying next format if available.".format(asset.resourceName, e.getMessage))
        }
      }
      loadedSound.getOrElse(throw lastFailure.getOrElse(new ResourceNotFoundException(PartsResourcePath(assets.head.resourceName.split('/').toVector))))
    }

    private def loadSingleSound(asset: sgl.assets.AudioAsset): Sound = {
      logger.info("Loading sound resource: " + asset.resourceName)
      val resourceAudioStream = loadAudioInputStream(asset)
      val (convertedAudioStream, _) = convertToBestStream(resourceAudioStream).getOrElse{
        throw new ResourceFormatUnsupportedException(PartsResourcePath(asset.resourceName.split('/').toVector))
      }
      val wrapper = AudioInputStreamWrapper.fromAudioInputStream(convertedAudioStream)
      if(wrapper.frameLength <= 0) {
        logger.warning("Sound resource %s decoded to an empty audio stream.".format(asset.resourceName))
        throw new ResourceFormatUnsupportedException(PartsResourcePath(asset.resourceName.split('/').toVector))
      }
      new Sound(wrapper, 0)
    }
    ...
    class Music(clip: Clip) extends AbstractMusic {
      ...
      override def play(): Unit = {
        ...
        if(shouldLoop)
          ...
          clip.start()
        logger.debug("Started Java Sound music clip: running=%s active=%s frameLength=%d".format(clip.isRunning, clip.isActive, clip.getFrameLength))
      }
    ...
    override def loadMusic(asset: sgl.assets.AudioAsset, extras: sgl.assets.AudioAsset*): Loader[Music] = FutureLoader {
      logger.info("Loading music resource: " + asset.resourceName)
      val resourceAudioStream = loadAudioInputStream(asset)
    ...
    private def loadAudioInputStream(asset: sgl.assets.AudioAsset): AudioInputStream = {
  ...
  override val Audio: JavaSoundAudio.type = JavaSoundAudio
    ...
    def setClipVolume(clip: Clip, volume: Float): Unit = {
      ...
      if(clip.isControlSupported(FloatControl.Type.MASTER_GAIN)) {
        ...
        val gainDB: Float = {
          ...
          val tmp = 20f*scala.math.log10(volume).toFloat
