...
trait CordovaNativeAudioAudioProvider extends AudioProvider { this:
  ...
  object CordovaNativeAudioAudio extends Audio {
    ...
    override def loadSound(asset: sgl.assets.AudioAsset, extras: sgl.assets.AudioAsset*): Loader[Sound] = {
      ...
      val bestPath = paths.find(p => SupportedSoundFormats.contains(p.extension.getOrElse(""))).getOrElse(paths.head)
    ...
    override def loadMusic(asset: sgl.assets.AudioAsset, extras: sgl.assets.AudioAsset*): Loader[Music] = {
      val paths = (asset +: extras).map(a => ResourcesRoot / a.resourceName)
      val bestPath = paths.find(p => SupportedMusicFormats.contains(p.extension.getOrElse(""))).getOrElse(paths.head)
