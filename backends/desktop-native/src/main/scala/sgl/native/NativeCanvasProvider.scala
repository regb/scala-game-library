...
trait NativeCanvasProvider extends CanvasProvider {
  ...
  object NativeGraphics extends Graphics {
    ...
    override def loadImage(asset: sgl.assets.DrawableAsset): Loader[Bitmap] = {
      val path = nativeAssetPath(asset.resourceNameForDensity("mdpi"))
      val surface: Ptr[SDL_Surface] = Zone.acquire { implicit z => IMG_Load(toCString(path)) }
      if(surface == null) {
        Loader.failed(new Exception("Error while loading image %s: %s".format(path, fromCString(SDL_GetError()))))
      } else {
        val width = surface.w
        ...
        // Canvas/Tiled rendering often draws sub-regions from a texture atlas.
        // Linear filtering samples across the exact tile-region border and can
        // bleed pixels from the atlas spacing/margins, showing seams between
        // adjacent tiles. Use nearest filtering for the legacy native Canvas
        // backend; callers that need smooth scaling should opt into a renderer
        // with explicit filtering control.
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, 0x2600)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, 0x2600)
        SDL_FreeSurface(surface)
        val texture: Bitmap = new OpenGLTextureBitmap(!textureId, width, height)
        Loader.successful[Bitmap](texture)
      }
    }

    final class OpenGLTextureBitmap(initialTexture: GLuint, val width: Int, val height: Int) extends AbstractBitmap {
      private var textureValue = initialTexture

      def texture: GLuint = {
        if(textureValue == 0.toUInt) throw new IllegalStateException("Trying to use a released bitmap")
        textureValue
      }

      override def release(): Unit = if(textureValue != 0.toUInt) {
        val textureId = stackalloc[GLuint]()
        !textureId = textureValue
        glDeleteTextures(1.toUInt, textureId)
        textureValue = 0.toUInt
      }
    }
    ...
    object NativeFontCompanion extends FontCompanion {
      ...
      override def load(asset: sgl.assets.FontAsset): Loader[Font] = Loader.successful(Default)
    ...
    class NativeCanvas extends AbstractCanvas {
      ...
      override def drawOval(x: Float, y: Float, width: Float, height: Float, paint: Paint): Unit = {
        ...
        while(degree < 360) {
          ...
          glVertex2d(scala.math.cos(radian)*width/2, scala.math.sin(radian)*height/2)
          degree += 1
        }
        glEnd()
