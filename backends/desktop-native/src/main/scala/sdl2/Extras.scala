...
/* Define a four character code as a Uint32 */
  ...
  def SDL_SetError(fmt: CString, args: CVarArg*): CInt = -1
  ...
  def SDL_LoadWAV(file: CString, spec: Ptr[SDL_AudioSpec], audio_buf: Ptr[Ptr[UByte]], audio_len: Ptr[UInt]): Ptr[SDL_AudioSpec] =
    SDL_LoadWAV_RW(SDL_RWFromFile(file, c"rb"), 1, spec, audio_buf, audio_len)

  implicit class SDL_AudioSpecOps(val self: Ptr[SDL_AudioSpec]) extends AnyVal {
    def freq: CInt = self._1
    def freq_=(v: CInt): Unit = self._1 = v
    def format: SDL_AudioFormat = self._2
    def format_=(v: SDL_AudioFormat): Unit = self._2 = v
    def channels: UByte = self._3
    def channels_=(v: UByte): Unit = self._3 = v
    def silence: UByte = self._4
    def samples: UShort = self._5
    def samples_=(v: UShort): Unit = self._5 = v
    def padding: UShort = self._6
    def size: UInt = self._7
    def callback: SDL_AudioCallback = self._8
    def callback_=(v: SDL_AudioCallback): Unit = self._8 = v
    def userdata: Ptr[Byte] = self._9
    def userdata_=(v: Ptr[Byte]): Unit = self._9 = v
  }

  val AUDIO_U8: SDL_AudioFormat = 0x0008.toUShort
  val AUDIO_S16LSB: SDL_AudioFormat = 0x8010.toUShort
  val AUDIO_S16MSB: SDL_AudioFormat = 0x9010.toUShort
  val AUDIO_S16SYS: SDL_AudioFormat = AUDIO_S16LSB

  val SDL_AUDIO_ALLOW_FREQUENCY_CHANGE: CInt = 0x00000001
  val SDL_AUDIO_ALLOW_FORMAT_CHANGE: CInt = 0x00000002
  val SDL_AUDIO_ALLOW_CHANNELS_CHANGE: CInt = 0x00000004
  val SDL_AUDIO_ALLOW_SAMPLES_CHANGE: CInt = 0x00000008
  val SDL_AUDIO_ALLOW_ANY_CHANGE: CInt = SDL_AUDIO_ALLOW_FREQUENCY_CHANGE | SDL_AUDIO_ALLOW_FORMAT_CHANGE | SDL_AUDIO_ALLOW_CHANNELS_CHANGE | SDL_AUDIO_ALLOW_SAMPLES_CHANGE
