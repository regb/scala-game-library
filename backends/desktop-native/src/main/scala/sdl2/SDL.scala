...
object SDL {
  ...
  def SDL_LoadWAV_RW(src: Ptr[SDL_RWops], freesrc: CInt, spec: Ptr[SDL_AudioSpec], audio_buf: Ptr[Ptr[UByte]], audio_len: Ptr[UInt]): Ptr[SDL_AudioSpec] = extern
  def SDL_FreeWAV(audio_buf: Ptr[UByte]): Unit = extern
