...
def _tiled_export_impl(ctx):
    ...
    for src_file in ctx.files.srcs:
        ...
        ctx.actions.run_shell(
            ...
            tools = [ctx.executable._tiled, ctx.attr._tiled_data[DefaultInfo].files],
            outputs = [output_file],
            command = "\"$1\" --export-map --embed-tilesets --detach-templates --resolve-types-and-properties json \"$2\" \"$3\"",
            arguments = [ctx.executable._tiled.path, src_file.path, output_file.path],
...
_tiled_export = rule(
    ...
    attrs = {
        ...
        "_tiled": attr.label(default = "@tiled_tool//:tiled_wrapper.sh", executable = True, cfg = "exec", allow_single_file = True),
        "_tiled_data": attr.label(default = "@tiled_tool//:tiled_extracted", cfg = "exec"),
    },
    doc = "Processes Tiled map files and exports them with embedded tilesets and resolved properties",
)

def tiled_export(name, **kwargs):
    _tiled_export(
        name = name,
...
def _validate_embedded_value(value, description):
    for unsupported in ["|", "'", "\n", "\r"]:
        if unsupported in value:
            fail("%s contains an unsupported manifest character: %s" % (description, value))

def _sgl_tiled_import_impl(ctx):
    _validate_embedded_value(ctx.attr.project_root, "project_root")
    _validate_embedded_value(ctx.attr.output_prefix, "output_prefix")
    source_scale = _DENSITY_SCALE[ctx.attr.source_density]
    outputs = []
    image_output_lines = []
    image_outputs = []
    for image in ctx.files.images:
        _validate_embedded_value(image.short_path, "Tiled image path")
        if image.extension.lower() == "webp":
            if ctx.attr.tile_extrude:
                fail("Tiled WebP images cannot be tile-extruded; use PNG/JPEG: %s" % image.short_path)
            for density in ctx.attr.target_densities:
                if density != ctx.attr.source_density:
                    fail("Tiled WebP images cannot be density-resized; use PNG/JPEG or only request the source density: %s" % image.short_path)
    ...
    for src_file in ctx.files.maps:
        _validate_embedded_value(src_file.path, "Tiled map path")
    ...
    ctx.actions.run_shell(
        ...
        tools = [ctx.executable._tiled, ctx.attr._tiled_data[DefaultInfo].files, ctx.executable._image_tool],
        outputs = outputs,
        command = """set -e
export TILED='{tiled_path}'
export IMAGE_TOOL='{image_tool_path}'
python3 - <<'PY'
...
def convert_resize(src, density, out):
    ...
    if density == source_density:
        ...
        subprocess.check_call([os.environ['IMAGE_TOOL'], 'resize', src, str(percent), out])

def extrude_tileset(src, out, tw, th, columns, tilecount, margin, spacing, extrude):
    os.makedirs(os.path.dirname(out), exist_ok=True)
    if extrude <= 0:
        shutil.copyfile(src, out); return
    transparent = str(out.lower().endswith('.png')).lower()
    subprocess.check_call([
        os.environ['IMAGE_TOOL'], 'extrude', src, out, str(tw), str(th), str(columns),
        str(tilecount), str(margin), str(spacing), str(extrude), transparent,
    ])

# Export maps and collect tileset metadata before deciding whether each source
# image can be extruded. One atlas can be shared by incompatible tileset grids.
tileset_meta = {{}}
tileset_references = []
exported_maps = []
for line in map_lines:
    map_path, out_path = line.split('|')
    subprocess.check_call([os.environ['TILED'], '--export-map', '--embed-tilesets', '--detach-templates', '--resolve-types-and-properties', 'json', map_path, out_path])
    ...
    for ts in data.get('tilesets', []):
        if isinstance(ts.get('image'), str):
            ...
            if resolved not in tileset_meta:
                tileset_meta[resolved] = metadata
            elif tileset_meta[resolved] != metadata:
                tileset_meta[resolved] = None
            tileset_references.append((ts, resolved))
    ...
    rewrite_images(data)
    exported_maps.append((out_path, data))

for ts, resolved in tileset_references:
    if source_extrude_px > 0 and tileset_meta.get(resolved):
        ts['margin'] = source_extrude_px
        ts['spacing'] = 2 * source_extrude_px
        rows = (ts['tilecount'] + ts['columns'] - 1) // ts['columns']
        ts['imagewidth'] = ts['columns'] * (ts['tilewidth'] + 2 * source_extrude_px)
        ts['imageheight'] = rows * (ts['tileheight'] + 2 * source_extrude_px)

for out_path, data in exported_maps:
...
for src in image_paths:
    ...
    if meta:
        ...
        if source_extrude_px > 0 and src in tileset_meta:
            print('Skipping tile extrusion for shared image with incompatible tileset grids: ' + src, file=sys.stderr)
...
""".format(
            ...
            tiled_path = ctx.executable._tiled.path,
            image_tool_path = ctx.executable._image_tool.path,
        ),
...
def sgl_tiled_import(name, **kwargs):
    _sgl_tiled_import(
        name = name,
        target_compatible_with = ["@platforms//os:linux"],
        **kwargs
