...
trait AbstractApp extends OpenGLGameObjectApp with DefaultRandomProvider {
  ...
  override def configure(builder: GameBuilder): Unit = {
    builder
      ...
      .setContext(ClearColor2D(0.02f, 0.02f, 0.05f, 1f))
...
object Player {
  ...
  def fireBullet(context: SystemContext, ship: Spatial2D): Unit = {
...
object AsteroidsScene extends Scene {
  override def load(context: SceneContext)(using Scope): Unit = {
