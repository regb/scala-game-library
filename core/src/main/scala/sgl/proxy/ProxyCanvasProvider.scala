...
trait ProxyCanvasProvider extends CanvasProvider {
  ...
  object ProxyGraphics extends Graphics {
    ...
    override def loadImage(asset: sgl.assets.DrawableAsset): Loader[Bitmap] = {
      val resourceName = runtimeResourceName(asset.resourceNameForDensity("mdpi"))
      PlatformProxy.graphicsProxy.loadImage(resourceName).transform {
    ...
    object ProxyFontCompanion extends FontCompanion {
      ...
      override def load(asset: sgl.assets.FontAsset): Loader[Font] = {
        ...
        PlatformProxy.graphicsProxy.fontCompanionProxy.load(resourceName).transform {
        case Failure(_: ProxyResourceNotFoundException) => Failure(ResourceNotFoundException(PartsResourcePath(resourceName.split('/').toVector)))
        case other => other
        }.map(f => ProxyFont(f))
      }
