...
trait CanvasProvider extends GraphicsHelpersComponent {
  this: SystemProvider =>
  ...
  trait Graphics extends GraphicsExtension {
    ...
    /** Load a bitmap identified by a resource path.
      ...
      */
    def loadImage(asset: DrawableAsset): Loader[Bitmap]
    ...
    abstract class FontCompanion {
      ...
      /** Load a font from a typed font asset.
        *
        * This supports TrueType (.ttf), OpenType (.otf), and web font formats
        ...
        */
      def load(asset: FontAsset): Loader[Font]
  ...
  /** Provides access to the Canvas for the current frame.
    *
    * The canvas is only valid inside the dynamic scope of this call. Game code
    * must not store it and use it after `withFrameCanvas` returns.
    */
  def withFrameCanvas[A](f: Graphics.Canvas => A): A
