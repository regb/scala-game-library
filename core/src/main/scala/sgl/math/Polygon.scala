...
class Polygon(val vertices: Vector[Vec2]) {
  require(vertices.size > 2)

  def nbEdges: Int = vertices.size

  def edgeStart(i: Int): Vec2 = vertices(i)
  def edgeEnd(i: Int): Vec2 = vertices((i+1) % vertices.size)
...
object Polygon {
  def apply(vertices: Vector[Vec2]) = new Polygon(vertices)
}
