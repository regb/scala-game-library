...
trait AbstractApp extends Application with AssetsProvider {
  ...
  private def update(dt: Long): Unit = {
    ...
    totalTime += dt
    if(Input.isKeyPressed(Input.Keys.Left)) x -= 50f * (dt / 1000f)
    if(Input.isKeyPressed(Input.Keys.Right)) x += 50f * (dt / 1000f)
    if(Input.isKeyPressed(Input.Keys.Up)) y -= 50f * (dt / 1000f)
    if(Input.isKeyPressed(Input.Keys.Down)) y += 50f * (dt / 1000f)
    autoX += 50f * (dt / 1000f)
    autoY += 50f * (dt / 1000f)
  }
  ...
  private def render(canvas: Canvas): Unit = {
    ...
    canvas.withSave {
      ...
      canvas.drawBitmap(characterAnimation.currentFrame(totalTime), x, y, 160f / Window.logicalPpi, 0.5f)
